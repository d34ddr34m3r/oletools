...
# 2020/04/23 : Harli Aquino <maharlito.aquino@cyren.com, https://github.com/d34ddr34m3r/oletools>
# - slightly modified version to display cells and formulas as closely as how you see it in Excel
...
def Int2ColumnId(_input_num):
    _result_string = ''
    _base_n = 26
    while _input_num != 0:
        _remainder_num = _input_num % _base_n
        # if its Z do carry flag stuff
        if _remainder_num == 0:
            _input_num = _input_num - _base_n
            _remainder_num = _base_n
        # build result string
        _result_string = chr(64 + _remainder_num) + _result_string
        _input_num = int(_input_num / _base_n)
    ...
    while len(expression) > 0:
        ...
        if ptgid in dTokens:
            ...
            if ptgid in dMathOps:  # ptgAdd, ptsSub, ptgMul, ptgDiv
                ...
                bool_value = IFF(P23Ord(expression[0]), 'TRUE', 'FALSE')
                # result += '%s ' % (bool_value)
                expression = expression[1:]
                args.append(bool_value)
            elif ptgid == 0x1e:
                int_value = P23Ord(expression[0]) + P23Ord(expression[1]) * 0x100
                # result += '%d ' % (int_value)
                expression = expression[2:]
                args.append(str(int_value))
            elif ptgid in [0x21, 0x41, 0x61]:  # ptgFunc, ptgFuncV, ptgFuncA
...
class SheetName(object):
    def __init__(self, data):
        self.__data__ = data
        self.__cch__ = None
        self.__A__ = None
        self.__sheetname_ascii__ = None

    @property
    def cch(self):
        if self.__cch__ is None:
            self.__cch__ = self.__data__[0x06]
        return self.__cch__

    @property
    def A(self):
        if self.__A__ is None:
            self.__A__ = self.__data__[0x07]
        return self.__A__
...
class cBIFF(cPluginParent):
    ...
    def Analyze(self):
        ...
        if self.streamname in [['Workbook'], ['Book']]:
            ...
            oParser.add_option('-n', '--normalize', action='store_true', default=False, help='Show normalized Excel 4.0 macros')
            oParser.add_option('-F', '--formula', action='store_true', default=False, help='Show FORMULA code')
            ...
            while position < len(stream):
                ...
                if opcode == 0x06 and len(data) >= 21:
                    ...
                    expression = data[22:]
                    if column + 1 not in macro_cells:
                ...
                if opcode == 0x18 and len(data) >= 16:
                    defined_name = dDefinedNames._make(struct.unpack('HBBHHHBBBB', data[:14]))
                    option_flags = {flag_name: defined_name.OptionFlags & flag_bit for flag_bit, flag_name in dOptionFlags.items() if defined_name.OptionFlags & flag_bit}
                    line += ' - {}'.format(', '.join([key.capitalize() for key in option_flags.keys()])) if len(option_flags) else ''
                ...
                if options.find == '' and options.opcode == '' and not options.xlm or options.opcode != '' and options.opcode.lower() in line.lower() or options.find != '' and options.find in data or options.xlm and opcode in [0x06, 0x18, 0x85,
                                                                                                                                                                                                                                   0x207]:
            ...
            if options.xlm and filepassFound:
                ...
                from .xlm_parser import xlm_parse
                result.extend(xlm_parse(result, show_formula=options.formula))

        return result, suspicious_sheets, macro_cells
